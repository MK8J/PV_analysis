...
class teal_psf():
    ...
    def teals_method(self, LSF):
        ...
        psf_teal[len(LSF) - 1] = LSF[len(LSF) - 1] / \
            coefficient_matrix[len(LSF) - 1, len(LSF) - 1]
        # find all psf values
    ...
    def generate_coefficient_matrix(self, len_lsf):
        ...
        for counter1 in range(0, len_lsf):
            ...
            for counter2 in range(counter1, len_lsf):
                r_min = (counter2 - 0.5)
                r_max = (counter2 + 0.5)
                coefficient_matrix[counter1, counter2] = 2.0 * \
                    self.calculate_enclosed_area(x_min, x_max, r_min, r_max)
    ...
    def calculate_enclosed_area(self, x_min, x_max, r_min, r_max):
        ...
        if r_max == x_max:
            ...
            arctan_expression_max_max = r_max**2 * \
                np.arctan(x_max / np.sqrt(r_max**2 - x_max**2))
        ...
        if r_max == x_min:
            ...
            arctan_expression_max_min = r_max**2 * \
                np.arctan(x_min / np.sqrt(r_max**2 - x_min**2))
        ...
        if r_min == x_max:
            ...
            arctan_expression_min_max = r_min**2 * \
                np.arctan(x_max / np.sqrt(r_min**2 - x_max**2))
        ...
        if r_min == x_min:
            ...
            arctan_expression_min_min = r_min**2 * \
                np.arctan(x_min / np.sqrt(r_min**2 - x_min**2))
        ...
        if r_min < 0:
            area_small_circle = 0.
        # following if statement prevents nan error when range of integral does
        # not include positive yvalue of circle
        if r_min == x_min:
            area_small_circle = 0.
        if r_max <= x_min:
            enclosed_area = 0.
