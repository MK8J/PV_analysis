...
def _openpyxl_Sinton2014_ExtractRawDatadata(wb):
    ...
    values1 = np.array([[i.value for i in j] for j in ws["A9:I133"]],
                       dtype=np.float64)
    headers1 = tuple(
        [[j.value.encode('utf8') for j in i] for i in ws["A8:I8"]][0])

    # get second section of data
    values2 = np.array([[i.value for i in j] for j in ws["O9:P133"]],
                       dtype=np.float64)
    headers2 = tuple(
        [[j.value.encode('utf8') for j in i] for i in ws["O8:P8"]][0])
...
#     fnames = [r'/home/shanoot/Temp,
# while own clod updates/B1-Diff-SiNx.xlsm',
