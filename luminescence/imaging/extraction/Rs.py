...
import analysis.util.util as util
...
def Kampwerth2008(images):
    ...
    Fit = np.polyfit(Vt[2:4], image_data[2:4], 2).reshape((2,
     image_data.shape[0],
      image_data.shape[1]))

    # the voltage at which the pixel are the same brightness is then
    Ut_2 = (-FitVals[1] + np.sqrt(FitVals[1]**2-4*FitVals[0] * (FitVals[2]-image_data[0]))/(2*FitVals[0])
